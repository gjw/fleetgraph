...
- **Type hints on all functions** (mypy strict is on)
- **Pydantic models at all API boundaries** (request, response, tool results)
- **No bare `except:`** — catch specific exceptions
- **No `# type: ignore`** without justification comment
- **Async for all external API calls** (httpx or native async clients)
- **No hardcoded URLs, credentials, or API keys** — everything from env vars or config

## Dependency Management

Managed with `uv`. Add with `uv add <package>`. Run with `uv run`.
...
- `uv run ruff format src/ tests/` passes (or `--check`)
- No bare except, no type: ignore without justification
- No hardcoded secrets or credentials

## Change Doc Extras

- **Migration notes** — any database migrations, schema changes, or env var additions
...
Include the issue ID in commit messages: `Fix auth validation (fg-a1b2)`
